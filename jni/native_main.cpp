...
//    performanceAnalyzer->log();
    ...
    if (!init) {
        ...
        query_image.create(frame_size.height / query_scale, frame_size.width / query_scale,
                           CV_8UC1);
        init = true;
...
//    performanceAnalyzer->count("init");
    ...
    if (!recog_result.empty()) {
        ...
        std::vector<cv::Point2f> position = recog_result[0].object_position;
        LOGI("p1: [%f, %f], p2: [%f, %f], p3: [%f, %f], p4: [%f, %f]",
             position.at(0).x, position.at(0).y,
             position.at(1).x, position.at(1).y,
             position.at(2).x, position.at(2).y,
             position.at(3).x, position.at(3).y
        );
